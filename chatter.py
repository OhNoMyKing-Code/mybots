...
class Chatter:
    def __init__(
        ...
        # Custom greetings
        self.player_greeting = self._format_message(
            "Hi {opponent}, I'm {me}, running {engine} on {cpu} with {ram} RAM. Good luck and have fun!"
        )
        self.player_goodbye = self._format_message(
            "GG {opponent}! That was fun. This bot is powered by {engine}. Come back anytime for a rematch!"
        )
        self.spectator_greeting = self._format_message(
            "Welcome to the game! Watch {me} using {engine} in action. Powered by {cpu} & {ram} 💻"
        )
        self.spectator_goodbye = self._format_message(
            "Thanks for watching! That was a cool game!"
        )
    ...
    async def send_abortion_message(self) -> None:
        await self.api.send_chat_message(self.game_info.id_, 'player',
            "Too bad you weren't there. Feel free to challenge me again. I'll accept if possible!")
    ...
    def _append_pv(self, initial_message: str = '') -> str:
        ...
        board = self.lichess_game.board.copy(stack=1) if self.lichess_game.is_our_turn else self.lichess_game.board.copy(stack=False)
